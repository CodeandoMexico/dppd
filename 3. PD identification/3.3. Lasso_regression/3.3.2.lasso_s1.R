...
# elección de variables para cluster 1
...
x <- model.matrix(g1_log~., clus_1)[,-1]
y <- clus_1 %>% 
  dplyr::select(g1_log) %>% 
  unlist() %>% 
  as.numeric()
...
x_train <- model.matrix(g1_log~., train)[,-1]
x_test <- model.matrix(g1_log~., test)[,-1]
...
data.frame(
  RMSE = RMSE(predict, clus_1$g1_log),
  Rsquare = R2(predict, clus_1$g1_log)
)
...
data <- data %>% 
  mutate(residuals = predict- g1_log) %>% 
  arrange(-residuals)
hist(data$residuals)
...
PD_g1_c1 <- left_join(PD_g1_c1, base[c("CVEGEO", "cluster")], by = "CVEGEO")
...
# Crear matrices de x (variables independientes) y y (variable dependiente)
x <- model.matrix(g1_log~., clus_2)[,-1]
y <- clus_2 %>% 
  dplyr::select(g1_log) %>% 
  unlist() %>% 
  as.numeric()
...
# Calcular RMSE y R2
data.frame(
  RMSE = RMSE(predict, clus_2$g1_log),
  Rsquare = R2(predict, clus_2$g1_log)
)
...
PD_g1_c2 <- left_join(PD_g1_c2, base[c("CVEGEO", "cluster")], by = "CVEGEO")
...
# Crear matrices de x (variables independientes) y y (variable dependiente)
x <- model.matrix(g1_log~., clus_3)[,-1]
y <- clus_3 %>% 
  dplyr::select(g1_log) %>% 
  unlist() %>% 
  as.numeric()
...
# Calcluar RMSE y R2
data.frame(
  RMSE = RMSE(predict, clus_3$g1_log),
  Rsquare = R2(predict, clus_3$g1_log)
)
...
PD_g1_c3 <- left_join(PD_g1_c3, base[c("CVEGEO", "cluster")], by = "CVEGEO")

# Crear una sola base de resultados y guardarla 
PD_g1_lasso <- bind_rows(PD_g1_c1, PD_g1_c2, PD_g1_c3)

PD_g1_lasso <- PD_g1_lasso %>% 
  mutate(model = "lasso",
         result_var = "g1_log")

write.csv(PD_g1_lasso, "pd_g1_lasso.csv", row.names = F)
