...
# Correr la regresión lineal para la variable dependiente g1_log, y en las independientes incluir la variable cluster.
...
# Residuos modelo 1 
res_mod <- cbind.data.frame(base$CVEGEO, base$cluster, base$g1_log, predict(model1), model1$residuals)

# Obtener los PDS por clusters para el modelo 1

# Separar los residuos del modelo 1 para el cluster 1 
...
# Identificar los PDS de los residuos del modelo 1 cluster 1
...
# Comprobación de supuestos 
shapiro.test(model_c1$residuals)# Se cumple el supuesto de normalidad
...
# Identificar PDS para el modelo cluster 1
Q1<- quantile(res_mod_c1$`model_c1$residuals`, 0.25)
iq.data <- IQR (res_mod_c1$`model_c1$residuals`)
b<-Q1-(iq.data*1.5)
b

PD_g1_c1 = res_mod_c1[which(res_mod_c1$`model_c1$residuals` <b), ]
PD_g1_c1
...
# Correr el modelo para cluster 2
# No se incluye P_FEM, P_MAS, P_60YMAS y serv_personales_km2 para eliminar la multicolinealidad alta. 

model_c2 <- lm(g1_log ~ POBTOT+
                 area_agebkm+
                 P_0A14+
                 P_15A24+
                 P_25A59+
...
# Identificar PDs modelo cluster 2
Q1<- quantile(res_mod_c2$`model_c2$residuals`, 0.25)
iq.data <- IQR (res_mod_c2$`model_c2$residuals`)
b<-Q1-(iq.data*1.5)
b

PD_g1_c2= res_mod_c2[which(res_mod_c2$`model_c2$residuals` <b), ]
PD_g1_c2
...
# Correr el modelo cluster 3 sin la variable de paradassitis_km2 porque causa multicolinealidad perfecta (ya que no hay paradas SITIS en el cluster 3).
...
PD_g1_c3 = res_mod_c3[which(res_mod_c3$`model_c3$residuals` <b), ]
PD_g1_c3
...
PD_g1_linearreg <- bind_rows(PD_g1_c1, PD_g1_c2, PD_g1_c3)

PD_g1_linearreg <- PD_g1_linearreg %>% 
  mutate(model = "linearreg",
         result_var = "g1_log")

write.csv(PD_g1_linearreg, "pd_g1_linearreg.csv", row.names = F)
