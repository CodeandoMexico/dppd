...
# Mejor modelo 1
best_mod<- glm.nb(g2 ~ area_agebkm +
                    distanciamintransporte_km +
                    POBTOT +
...
# PDS
res_mod <- cbind.data.frame(base$CVEGEO, base$cluster, base$g2, residuals(best_mod))
...
# Modelo globla cluster 1

options(na.action="na.fail")

globmod_c1<- MASS::glm.nb(g2 ~ POBTOT+ 
                            area_agebkm+
                            P_0A14+
                            P_15A24+
                            P_25A59+
...
dd1_g2 <-dredge(globmod_c1,  m.lim=c(0,4))

# Mejor modelo cluster 1

summary(best_mod_c1<- glm.nb(g2 ~ area_agebkm +
                               distanciamintransporte_km +
                               POBTOT +
...
b<-Q1-(iq.data*1.5)
b
pd_g2_c1 = res_mod_c1[which(res_mod_c1$residuals <b), ]
pd_g2_c1
...
options(na.action="na.fail")
globmod_c2<- MASS::glm.nb(g2 ~ POBTOT+ 
                            area_agebkm+
                            P_0A14+
                            P_15A24+
                            P_25A59+
...
dd2_g2<-dredge(globmod_c2,  m.lim=c(0,4))

# Mejor modelo cluster 2

best_mod_c2<- glm.nb(g2 ~ ambulantaje_por +
                       distanciamintransporte_km +
                       POBTOT + 
                       serv_financieros_km2, data=clus_2)
...
b<-Q1-(iq.data*1.5)
b
pd_g2_c2 = res_mod_c2[which(res_mod_c2$residuals <b), ]
pd_g2_c2
...
options(na.action="na.fail")
globmod_c3<- MASS::glm.nb(g2 ~ POBTOT+ 
                            area_agebkm+
                            P_0A14+
                            P_15A24+
                            P_25A59+
...
dd3_g2<-dredge(globmod_c3,  m.lim=c(0,4))

# Mejor modelo cluster 3

best_mod_c3<- glm.nb(g2 ~  ambulantaje_por +
                       area_agebkm + 
                       serv_preparacionalimentos_km2 +
                       sinpavimento_por, data=clus_3)
...
b<-Q1-(iq.data*1.5)
b
pd_g2_c3 = res_mod_c3[which(res_mod_c3$`residuals(best_mod_c3)` <b), ]
pd_g2_c3
...
pd_g2_nb <- bind_rows(pd_g2_c1, pd_g2_c2, pd_g2_c3)

pd_g2_nb <- pd_g2_nb %>% 
  mutate(model = "nb",
         result_var = "g2")

write.csv(pd_g2_nb, "pd_g2_nb.csv", row.names = F)
